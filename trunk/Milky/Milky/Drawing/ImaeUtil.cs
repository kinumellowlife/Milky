...
namespace Milky.Drawing
{
	...
	public class BitmapFilter
	{
		...
		/// <summary></summary>
		public const short EDGE_DETECT_ISOTROPIC = 4;
		...
		/// <param name="nWeight"></param>
		/// <returns></returns>
		public static bool ave(Bitmap b, int nWeight /* default to 9*/ )
		{
			ConvMatrix m = new ConvMatrix();
			m.SetAll(1);
			m.Factor = 9;
		...
		/// <param name="b"></param>
		/// <returns></returns>
		public static bool MedianFilter(Bitmap b)
		{
		...
		public static bool EdgeDetectConvolution(Bitmap b, short nType, byte nThreshold)
		{
			...
			{//huong ngang
				...
				case EDGE_DETECT_KIRSH://huong tay
					m.SetAll(-3);
					m.Pixel = 0;
				...
				case EDGE_DETECT_ISOTROPIC:
					m.SetAll(0);
					m.TopLeft = m.BottomLeft = -1;
					m.TopRight = m.BottomRight = 1;
					m.MidLeft = -(int)(Math.Sqrt(2));
					m.MidRight = (int)Math.Sqrt(2);
					m.Offset = 0;
					break;
			}
			...
			{//huong doc
				...
				case EDGE_DETECT_KIRSH://huong nam
					m.SetAll(-3);
					m.Pixel = 0;
				...
				case EDGE_DETECT_ISOTROPIC:
					...
					m.TopLeft = m.TopRight = -1;
					m.BottomLeft = m.BottomRight = 1;
					m.TopMid = -(int)(Math.Sqrt(2));
					m.BottomMid = (int)Math.Sqrt(2);
					m.Offset = 0;
					break;
			}
			...
			int stride = bmData.Stride;//lay tong so byte chua tren 1 dong
			System.IntPtr Scan0 = bmData.Scan0;
			System.IntPtr Scan02 = bmData2.Scan0;
			...
			unsafe
			{
				...
				int nOffset = stride - b.Width * 3;//tong so byte thuc hcua tren 1 dong - so byte chua du lieu anh
				int nWidth = b.Width * 3;//width= so pixel chieu rong cua anh - nhan 3 la moi pixel gom 3 byte
		...
		/// <param name="m"></param>
		/// <returns></returns>
		public static bool EdgeDetectConCustom(Bitmap b, ConvMatrix m)
		{
			// I need to make a copy of this bitmap BEFORE I alter it 80)
			byte nThreshold = (byte)m.Offset;
			ConvMatrix mt = new ConvMatrix();
			Bitmap bTemp = (Bitmap)b.Clone();

			BitmapFilter.Conv3x3(b, m);

			//Hoan vi ma tran mat na
			mt.TopLeft = m.TopLeft;
			mt.TopMid = m.MidLeft;
			mt.TopRight = m.BottomLeft;
			mt.MidLeft = m.TopMid;
			mt.Pixel = m.Pixel;
			mt.MidRight = m.BottomMid;
			mt.BottomLeft = m.TopRight;
			mt.BottomMid = m.MidRight;
			mt.BottomRight = m.BottomRight;

			BitmapFilter.Conv3x3(bTemp, mt);
		...
		/// <returns></returns>
		public static bool Kuwahara(Bitmap bmp, int block)
		{
			...
			for (int y = 0; y < h; y++)
				for (int x = 0; x < w; x++)
				{
					...
					dst.R = ImgUtils.AdjustByte(r / num);
					dst.G = ImgUtils.AdjustByte(g / num);
					dst.B = ImgUtils.AdjustByte(b / num);
				}
	...
	public static class ImgUtils
	{
		...
		public static Bitmap GetPalette(Bitmap bmp)
		{
			...
			Bitmap palb = new Bitmap(r * 16 + m * 2 + 1, r * 16 + m * 2 + 1,
		...
		/// <param name="zone"></param>
		/// <returns></returns>
		public static bool GaussianBlur(Bitmap bmp, int zone)
		{
			if (bmp.PixelFormat != PixelFormat.Format24bppRgb)
				return false;
		...
		/// <param name="fGrayScale"></param>
		/// <returns></returns>
		public static bool Laplacian(Bitmap bmp, bool fGrayScale)
		{
			int[,] mask = { { 0, -1,  0},
							{-1,  4, -1},
							{ 0, -1,  0} };
		...
		/// <param name="fGrayScale"></param>
		/// <returns></returns>
		public static bool LaplacianCustom(Bitmap bmp, ConvMatrix m, bool fGrayScale)
		{
			int[,] mask = { {m.TopLeft,m.TopMid,m.TopRight},
		...
		/// <param name="fGrayScale"></param>
		/// <returns></returns>
		public static bool LoG(Bitmap bmp, bool fGrayScale)
		{
			int[,] mask = { { 0,0,1,0,0},
							{ 0,1,2,1,0},
							{1,2,-16,2,1},
		...
		public static bool Pixelate2(ref Bitmap bmp, int zone)
		{
			...
			for (int y = 0; y < h; y += zone)
			{
				...
				for (int x = 0; x < w; x += zone)
				{
					...
					if (sumr > 127) { rrr = (byte)(sumr * 0.8); }
					else { rrr = (byte)(sumr * 1.2); }

					if (sumg > 127) { ggg = (byte)(sumg * 0.8); }
					else { ggg = (byte)(sumg * 1.2); }

					if (sumb > 127) { bbb = (byte)(sumb * 0.8); }
					else { bbb = (byte)(sumb * 1.2); }
		...
		/// <param name="bright"></param>
		/// <returns></returns>
		public static bool Oscillo(Bitmap bmp, int width, bool bHorizontal,
														Color dark, Color bright)
		{
			if (bmp.PixelFormat != PixelFormat.Format24bppRgb)
				return false;
		...
		/// <param name="block"></param>
		/// <returns></returns>
		public static bool KuwaharaMedian(Bitmap bmp, int block)
		{
			if (bmp.PixelFormat != PixelFormat.Format24bppRgb)
				return false;
		...
		/// <param name="block"></param>
		/// <returns></returns>
		public static bool KuwaharaOilPaint(Bitmap bmp, int block)
		{
			if (bmp.PixelFormat != PixelFormat.Format24bppRgb)
				return false;
		...
		/// <returns></returns>
		public static bool KuwaharaGrayScale(Bitmap bmp,
			...
			for (int y = 0; y < h; y++)
				for (int x = 0; x < w; x++)
				{
					...
					if (bDetail)
					{
						...
						dst[x, y] = ImgUtils.AdjustByte(t * (d / num) +
														(1d - t) * dst[x, y]);
					}
					else
						dst[x, y] = ImgUtils.AdjustByte(d / num);
				}
				...
				bool fStretch, Color dark, Color bright, out Bitmap bmp1)
		{
			...
			ImgUtils.GrayScale(ref tmp);

			if (fStretch) ImgUtils.HistoStretch(ref tmp);
				...
				for (int x = 0; x < w; x++)
				{
					...
					for (int iy = 0; iy < row; iy++)
						for (int ix = -xrange; ix <= xrange; ix++)
						{
							...
							src[xx, yy] = ImgUtils.AdjustByte(src[xx, yy] +
														   err * df[iy, ix + xrange]);
						}
				}
										...
										double radius)
		{
			...
			for (int y = 0; y < h; y++)
				for (int x = 0; x < w; x++)
				{
					...
					if (rct.Contains(new PointF((float)xx, (float)yy)))
						ImgUtils.intBilinear(dst, src, x, y, xx, yy);
				}
			...
			for (int y = 0; y < h; y++)
				for (int x = 0; x < w; x++)
				{
					...
					switch (ro)
					{
						...
						case RankOp.Mid:
							dst.R = ImgUtils.AdjustByte((rmax + rmin) / 2);
							dst.G = ImgUtils.AdjustByte((gmax + gmin) / 2);
							dst.B = ImgUtils.AdjustByte((bmax + bmin) / 2);
							break;
												 ...
												 int radius, double angle)
		{
			...
			angle = -angle * 30d;
			Bitmap tmp2 = ImgUtils.BitmapRotate(tmp, (float)angle, Color.Green);
			tmp.Dispose();
		...
		public static bool AlphaSheet(ref Bitmap bmp, bool stretch, bool bInvert)
		{
			...
			ImgUtils.GrayScale(ref bmp);
			if (stretch) ImgUtils.HistoStretch(ref bmp);
		...
		public static bool SaturationHistogram(Bitmap bmp, out Bitmap bmpHist)
		{
			...
			for (int y = 0; y < h; y++)
				for (int x = 0; x < w; x++)
				{
					...
					ImgUtils.RGBToHSL(rr, gg, bb, out hue, out satu, out lumi);
								...
								int dstX, int dstY, double srcX, double srcY)
		{
			...
			dst[dstX, dstY, eRGB.a] = ImgUtils.AdjustByte(aa);
